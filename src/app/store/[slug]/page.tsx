...
export default function StorefrontPage({ params }: { params: { slug: string } }) {
  ...
  async function toggleWishlist(productId: string) {
    ...
    if (wishlist.includes(productId)) {
      await supabase.from('wishlists').delete().eq('customer_id', customer.id).eq('product_id', productId)
      setWishlist(prev => prev.filter(id => id !== productId))
    } else {
      await supabase.from('wishlists').insert({ customer_id: customer.id, product_id: productId, merchant_id: store.id })
  ...
  function removeItem(id: string, name?: string) {
    setCart(prev => {
      const newCart = prev.filter(i => !(i.product.id === id && (!name || i.product.name === name)))
      saveCartItem(newCart)
      return newCart
    })
  }
  ...
  const isService = store.business_type === 'services'
  const useCart = !isService && (store.order_mode === 'cart' || store.order_mode === 'both' || !store.order_mode)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Cart Drawer — hidden for service mode */}
      {!isService && cartOpen && (
                    ...
                    {!showWhatsAppForm ? (
                      ...
                      <div className="space-y-2 bg-gray-50 rounded-2xl p-3">
                        ...
                        <button onClick={async () => {
                          ...
                          if (rawPhone.startsWith('0')) localPhone = rawPhone.slice(1)
                          else if (waCountry.dial && rawPhone.startsWith(waCountry.dial)) localPhone = rawPhone.slice(waCountry.dial.length)
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {variantModal.variants!.map((v, i) => (
                    <button key={i} onClick={() => setSelectedVariantIndex(i)}
                      ...
                      className={`px-4 py-2 rounded-xl border-2 text-sm font-semibold transition-all ${selectedVariantIndex === i ? 'text-white border-transparent' : 'border-gray-200 text-gray-700 bg-white'} ${v.stock_qty === 0 ? 'opacity-40 cursor-not-allowed' : ''}`}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3 ml-auto">
                  <button onClick={() => setModalQty(q => Math.max(1, q - 1))} className="w-8 h-8 bg-gray-100 rounded-xl flex items-center justify-center"><Minus size={13} className="text-gray-600" /></button>
                  <span className="font-bold text-lg w-6 text-center">{modalQty}</span>
                  <button onClick={() => setModalQty(q => q + 1)} style={{ backgroundColor: store?.theme_color || '#1A7A4A' }} className="w-8 h-8 rounded-xl flex items-center justify-center"><Plus size={13} className="text-white" /></button>
                </div>
              </div>
              <div className="space-y-2 pt-1">
                <button
                  ...
                  className={`w-full font-bold py-3.5 rounded-2xl flex items-center justify-center gap-2 text-sm transition-all ${selectedVariantIndex === null ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'text-white'}`}>
                  <ShoppingCart size={16} />
                  {selectedVariantIndex !== null ? `Add ${modalQty} ${variantModal.variants![selectedVariantIndex].name} to Cart` : 'Select an option first'}
                </button>
                ...
                <button onClick={() => setVariantModal(null)} className="w-full text-sm text-gray-400 font-medium py-1">Continue Shopping</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Order/Booking Confirmed Banner */}
      {orderConfirmed && (
        <div style={{ backgroundColor: store.theme_color || '#1A7A4A', color: getContrastColor(store.theme_color || '#1A7A4A') }} className="fixed top-4 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-2xl shadow-xl flex items-center gap-3 max-w-sm mx-4">
          ...
          <div>
            <p className="font-bold text-sm">{isService ? 'Booking sent!' : 'Order sent!'}</p>
            <p className="text-xs opacity-80">{store?.business_name} will confirm via WhatsApp shortly.</p>
          </div>
          <button onClick={() => setOrderConfirmed(false)} className="ml-2 opacity-60 hover:opacity-100">✕</button>
        </div>
      )}
      ...
      <div style={{ backgroundColor: store.theme_color || '#1A7A4A' }} className="text-white">
        <div className="max-w-6xl mx-auto px-4 pt-5 pb-4">
          ...
          <div className="flex gap-6 mt-4">
            <div><span className="font-display font-bold text-xl">{products.length}</span><span className="text-xs opacity-60 ml-1">{isService ? 'Services' : 'Products'}</span></div>
            <div><span className="font-display font-bold text-xl">{products.filter(p => p.in_stock).length}</span><span className="text-xs opacity-60 ml-1">{isService ? 'Available' : 'In Stock'}</span></div>
          </div>
        </div>
        ...
        <div className="flex">
          <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"
            ...
            {WA_ICON}
            {isService ? 'Book via WhatsApp' : 'Chat with us'}
          </a>
          {isService ? (
            <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${store.business_name}! I'd like to know more about your services.`)}`}
                ...
                {customer ? (
                  <>
                    <div className="px-4 py-3 bg-brand-light border-b border-gray-100">
                      ...
                      {customerPoints > 0 && <p className="text-xs text-brand-green font-semibold mt-0.5">⭐ {customerPoints} loyalty points</p>}
                    </div>
                    <Link href={`/store/${store.slug}/account`} onClick={() => setAccountMenuOpen(false)}
                      ...
                      <ShoppingCart size={14} className="text-gray-400" /> {isService ? 'My Bookings' : 'My Orders'}
                    </Link>
                    <Link href={`/store/${store.slug}/account`} onClick={() => setAccountMenuOpen(false)}
                      className="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 border-b border-gray-50">
                      <Star size={14} className="text-gray-400" /> My Points & Rewards
                    </Link>
                    <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}?text=${encodeURIComponent('Hi! I need help.')}`}
        ...
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex gap-2 mb-2">
            <div className="relative flex-1">
              ...
              <input ref={searchRef} type="search" placeholder={isService ? 'Search services...' : 'Search products...'} value={search}
          ...
          <div className="flex gap-2 items-center">
            {(['all', 'instock'] as const).map(f => (
              <button key={f} onClick={() => { setActiveFilter(f); setPage(1) }}
                ...
                {f === 'all' ? (isService ? 'All Services' : 'All Products') : '✅ Available'}
              </button>
            ))}
            <span className="ml-auto text-xs text-gray-400">{filtered.length} {isService ? 'services' : 'products'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        {paginated.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 font-semibold mb-1">{search ? `No results for "${search}"` : (isService ? 'No services yet' : 'No products yet')}</p>
                  ...
                  <div className="relative aspect-square bg-gray-100 overflow-hidden">
                    ...
                    {!product.in_stock && (
                      <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
                        <span className="bg-red-100 text-red-600 text-xs font-bold px-3 py-1 rounded-full">{isService ? 'Unavailable' : 'Out of Stock'}</span>
                      </div>
                    )}
                    {/* Quick action overlay button */}
                    {product.in_stock && (
                      isService ? (
                        <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${store.business_name}! I'd like to book: ${product.name}`)}`}
                          target="_blank" rel="noreferrer"
                          style={{ backgroundColor: store.theme_color || '#1A7A4A' }}
                          className="absolute bottom-2 right-2 rounded-xl px-2.5 py-1 text-white text-xs font-bold shadow-lg">
                          Book
                        </a>
                      ) : useCart && (
                        <button onClick={() => addToCart(product)}
                          style={addedId === product.id ? {} : { backgroundColor: store.theme_color || '#1A7A4A' }}
                          className={`absolute bottom-2 right-2 w-8 h-8 rounded-xl flex items-center justify-center shadow-lg transition-all ${addedId === product.id ? 'bg-green-500 scale-110' : ''}`}>
                  ...
                  <div className="p-3">
                    ...
                    {product.in_stock ? (
                      <div className="space-y-1.5">
                        {isService ? (
                          ...
                          <button onClick={() => addToCart(product)}
                            ...
                            className={`w-full text-xs font-semibold py-2 rounded-xl flex items-center justify-center gap-1.5 transition-all ${addedId === product.id ? 'bg-green-500 text-white' : ''}`}>
                      ...
                      <button disabled className="w-full bg-gray-100 text-gray-400 text-xs font-semibold py-2 rounded-xl cursor-not-allowed">
                        {isService ? 'Unavailable' : 'Out of Stock'}
                      </button>
                    )}
                  </div>
                </div>
      ...
      {/* Floating cart — products only */}
      {!isService && useCart && cartCount > 0 && (
        <button onClick={() => setCartOpen(true)}
          style={{ backgroundColor: store.theme_color || '#1A7A4A', color: getContrastColor(store.theme_color || '#1A7A4A') }}
      ...
      {/* Floating Book button — services only */}
      {isService && (
        <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${store.business_name}! I'd like to make a booking.`)}`}
          target="_blank" rel="noreferrer"
          style={{ backgroundColor: store.theme_color || '#1A7A4A', color: getContrastColor(store.theme_color || '#1A7A4A') }}
          className="fixed bottom-6 right-6 font-bold px-5 py-3 rounded-2xl shadow-xl flex items-center gap-2 z-40">
          📲 Book via WhatsApp
        </a>
      )}
