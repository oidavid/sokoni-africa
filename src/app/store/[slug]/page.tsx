...
export default function StorefrontPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <StorefrontFooter
        store={store}
        themeColor={store.theme_preset ? (getThemeById(store.theme_preset)?.primary || store.theme_color || '#1A7A4A') : (store.theme_color || '#1A7A4A')}
        contrast={getContrastColor(store.theme_preset ? (getThemeById(store.theme_preset)?.primary || store.theme_color || '#1A7A4A') : (store.theme_color || '#1A7A4A'))}
        isLight={getContrastColor(store.theme_preset ? (getThemeById(store.theme_preset)?.primary || store.theme_color || '#1A7A4A') : (store.theme_color || '#1A7A4A')) === '#111827'}
        contextType="order"
