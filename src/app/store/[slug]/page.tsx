                    ...
                    {!showWhatsAppForm ? (
                      ...
                      <div className="space-y-2 bg-gray-50 rounded-2xl p-3">
                        ...
                        <button onClick={() => {
                          ...
                          const customerWa = rawPhone.startsWith('0') ? '234' + rawPhone.slice(1) : rawPhone.startsWith('234') ? rawPhone : '234' + rawPhone
                          // Send to merchant
                          window.open(`https://wa.me/${merchantWa}?text=${encodeURIComponent(merchantMsg)}`, '_blank')
                          // Send confirmation to customer - use same tab after delay to avoid popup block
                          if (customerWa.length >= 10) {
                            setTimeout(() => {
                              const link = document.createElement('a')
                              link.href = `https://wa.me/${customerWa}?text=${encodeURIComponent(customerMsg)}`
                              link.target = '_blank'
                              link.rel = 'noreferrer'
                  ...
                  <div className="p-3">
                    ...
                    {product.in_stock ? (
                      <div className="space-y-1.5">
                        ...
                        {useWhatsApp && !useCart && (
                          <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${store.business_name}! I want to order: *${product.name}* — ${formatPrice(product)}`)}`}
                        ...
                        {useWhatsApp && useCart && (
                          <a href={`https://wa.me/${store.whatsapp_number?.replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${store.business_name}! I want to order: *${product.name}* — ${formatPrice(product)}`)}`}
                            target="_blank" rel="noreferrer"
                            className="w-full flex items-center justify-center gap-1 text-xs font-semibold text-[#25D366] border border-[#25D366]/30 py-1.5 rounded-xl hover:bg-[#25D366]/5 transition-colors">
