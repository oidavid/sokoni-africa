  ...
  const loadOrders = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      if (oError) {
        ...
        setLoadError('Could not load orders: ' + oError.message)
      }
      setOrders(orderData || [])
    } catch (e) {
      console.error('Load error:', e)
      setLoadError('Something went wrong loading orders.')
    }
    setLoading(false)
    setRefreshing(false)
  }, [router])
  ...
  if (loadError) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4 px-4">
        ...
        <p className="text-gray-700 font-semibold text-center">{loadError}</p>
        <button onClick={() => loadOrders()} className="bg-brand-green text-white font-bold px-6 py-3 rounded-xl">
          Try Again
        </button>
        <Link href="/dashboard" className="text-brand-green font-semibold text-sm">← Back to Dashboard</Link>
      </div>
    )
  }
