...
function validateWhatsApp(raw: string, dialCode: string): { clean: string; error: string | null } {
  ...
  // normalizeNumber returns empty string or throws if invalid
...
export default function CashSalePage() {
  ...
  async function loadData() {
    ...
    // Find dial code from the same COUNTRIES list used in onboarding
    ...
    setProducts(prods || [])
    // ── Correct query: source=cash_pos, status=completed ──
  ...
  // Auto-lookup + validate WhatsApp as user types
  useEffect(() => {
